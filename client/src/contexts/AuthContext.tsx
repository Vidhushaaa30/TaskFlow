...
/**
 * Create client-side AuthContext and state management hook
 * Category: FEATURE
...
export const AuthContext: React.FC<AuthContextProps> = ({
  ...
  const handleExecute = () => {
    ...
    setTimeout(() => {
      setStatus('ready');
      if (onAction) {
        onAction({ action: 'execute', module: 'AuthContext', value: inputValue });
      }
    }, 300);
  };
  ...
  return (
    <div className={`p-5 rounded-xl border border-zinc-800 bg-zinc-900/60 backdrop-blur-md shadow-lg ${className}`}>
      ...
      <p className="text-xs text-zinc-400 mb-4 leading-relaxed">
        Store current user session, token persistence and login/logout handlers.
      </p>
